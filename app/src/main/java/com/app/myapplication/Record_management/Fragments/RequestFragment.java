...
import com.app.myapplication.Record_management.Fragments.Adapters.CustomAdapter_Request;
...
public class RequestFragment extends Fragment implements OnClickListener {
    ...
    ArrayList<RequestObj> listHistory;
    private JSONArray jsonArray_employee,jsonArray_requestHistory;
    private  String json;
    private  Employee entity;
    private RequestObj RequestHistory;
    //RECYCLE VIEW STUFF
    private RecyclerView recyclerView_Employees,recyclerView_Requests;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        view = inflater.inflate(R.layout.fragment_requests_dashboard, container, false);
        //initialization
        linearLayout = view.findViewById(R.id.Request_linear);
        linearLayout.setVisibility(View.INVISIBLE);

        //initialization of recycleView
        recyclerView_Employees = (RecyclerView) view.findViewById(R.id.recycleView_Request_EmpList);
            ...
            call.enqueue(new Callback() {
                public void onResponse(Call call, Response response) throws IOException {
                    ...
                    try {
                        ...
                        jsonArray_requestHistory = jsnobject.getJSONArray("History");
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    Gson gson = new Gson();
                    ...
                    for (int i = 0; i < jsonArray_requestHistory.length(); i++) {
                        try {
                            json = jsonArray_requestHistory.get(i).toString();
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                        RequestHistory = gson.fromJson(json, RequestObj.class);
                        //Creating Arraylist of Object task
                        listHistory.add(RequestHistory);
                    }
                    //Displaying Data
    ...
    private void DisplayDataFromHistoryArray(ArrayList<RequestObj> list){
        linearLayout.setVisibility(View.VISIBLE);
        //initialization of recycleView
        recyclerView_Requests = view.findViewById(R.id.RequestHistory_recycleView);
        //CustomAdapter for entry recycler_view
        CustomAdapter_Request customAdapterRequest = new CustomAdapter_Request(getActivity(),getActivity().getApplicationContext(), list);
        recyclerView_Requests.setAdapter(customAdapterRequest);
        recyclerView_Requests.setLayoutManager(new LinearLayoutManager(mContext));
    ...
    public void onCardClick(int id_) {
        ...
        //initialize data arrays
        listEmployee = new ArrayList<Employee>();
        listHistory = new ArrayList<RequestObj>();
